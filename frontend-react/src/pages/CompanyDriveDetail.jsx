...
export default function CompanyDriveDetail() {
  ...
  const loadResults = async () => {
    ...
    try {
      ...
      toast.success(`Loaded ${res.data.filtered_students} of ${res.data.total_students} students`);
    } catch (err) {
  ...
  const exportResults = async (format) => {
    try {
      const response = await api.get(`/company/drives/${driveId}/results/export`, {
        params: { format },
        responseType: 'blob'
      });
        ...
        <div className="flex-1 overflow-auto">
          <main className="py-8">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
              ...
              {/* Questions Tab */}
              {activeTab === 'questions' && (
                <div className="bg-linear-to-br from-white via-blue-50/30 to-indigo-50/30 rounded-2xl shadow-xl border border-gray-200/50 p-8">
                  <h2 className="text-2xl font-bold text-gray-900 mb-6">Questions Management</h2>
                  <p className="text-gray-600 mb-4">View and manage questions for this drive. Questions are displayed in the Overview tab.</p>
                  <button
                    onClick={() => setActiveTab('overview')}
                    className="bg-indigo-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-indigo-700 transition"
                  >
                    Go to Overview to Manage Questions
                  </button>
                </div>
              )}
              ...
              {activeTab === 'students' && (
                <div className="bg-linear-to-br from-white via-blue-50/30 to-indigo-50/30 rounded-2xl shadow-xl border border-gray-200/50 p-8">
                  <h2 className="text-2xl font-bold text-gray-900 mb-6">Students Management</h2>
                  <p className="text-gray-600 mb-4">View and manage students for this drive. Students list is displayed in the Overview tab.</p>
                  <button
                    onClick={() => setActiveTab('overview')}
                    className="bg-indigo-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-indigo-700 transition"
                  >
                    Go to Overview to Manage Students
                  </button>
                </div>
              )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                      ...
                      <div className="bg-linear-to-r from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-3">
                            Minimum Percentage: <span className="text-indigo-600 text-xl font-bold">{minPercentage}%</span>
                          </label>
                          <input
                            type="range"
                            min="0"
                        ...
                        >
                          {isLoadingResults ? '⏳ Loading...' : '🔍 Apply Filter & Load Results'}
                        </button>
                      </div>
                      ...
                      <div className="bg-linear-to-br from-green-50 to-emerald-50 rounded-lg p-6 border border-green-200">
                        ...
                        <p className="text-xs text-gray-500 mt-3">
                          <strong>Summary:</strong> Name, Email, Roll Number, College, Score, Percentage, Status<br/>
                          <strong>Detailed:</strong> All summary columns + individual question responses
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {isLoadingResults ? (
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-8 text-center">
                      ...
                      <p className="text-yellow-700">
                        Click "Apply Filter & Load Results" to view student results, or adjust the percentage filter.
                      </p>
                    </div>
                  ) : (
                    <div className="bg-white rounded-xl shadow-lg border border-gray-200/50 overflow-hidden">
                            ...
                            {results.map((result, index) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="px-6 py-4 text-gray-700">{result.email}</td>
                                  <td className="px-6 py-4 text-gray-700">{result.roll_number || 'N/A'}</td>
                                  <td className="px-6 py-4 text-gray-700">{result.college_name || 'N/A'}</td>
                                  <td className="px-6 py-4 text-gray-700">{result.student_group_name || 'N/A'}</td>
                                  <td className="px-6 py-4">
                                    <span className="font-semibold text-indigo-600">
                                      {result.score !== null ? result.score : '-'}/{result.total_marks !== null ? result.total_marks : '-'}
                                    </span>
                                  </td>
                                  <td className="px-6 py-4">
                                  ...
                                  <td className="px-6 py-4">
                                    {violationCount > 0 ? (
                                      <button
                                        onClick={() => {
                                          alert(`Violation Details:\n\n${JSON.stringify(result.violation_details, null, 2)}\n\nDisqualification Reason: ${result.disqualification_reason || 'N/A'}`);
                                        }}
                      ...
                      <div className="bg-gray-50 px-6 py-4 border-t border-gray-200">
                        <p className="text-sm text-gray-600 font-semibold">
                          <span className="text-indigo-600">📊</span> Showing {results.length} student(s)
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {results.length > 0 && (
                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                          <p className="font-semibold text-gray-700 mb-2">Status Colors:</p>
                          <ul className="space-y-1 text-gray-600">
                            <li>✅ <strong>Submitted:</strong> Exam completed successfully</li>
                            <li>⏳ <strong>In Progress:</strong> Currently taking the exam</li>
                            <li>⚪ <strong>Not Started:</strong> Haven't begun the exam</li>
                            <li>❌ <strong>Disqualified:</strong> Exceeded violation limits</li>
                          </ul>
                        </div>
                        <div>
                          <p className="font-semibold text-gray-700 mb-2">Percentage Colors:</p>
                          <ul className="space-y-1 text-gray-600">
                            <li><span className="inline-block w-4 h-4 bg-green-500 rounded mr-2"></span>80%+ : Excellent</li>
                            <li><span className="inline-block w-4 h-4 bg-blue-500 rounded mr-2"></span>60-79% : Good</li>
                            <li><span className="inline-block w-4 h-4 bg-yellow-500 rounded mr-2"></span>40-59% : Average</li>
                            <li><span className="inline-block w-4 h-4 bg-red-500 rounded mr-2"></span>&lt;40% : Below Average</li>
                          </ul>
                        </div>
                      </div>
                    </div>
